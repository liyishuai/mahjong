...
let is_valid_target (tiles : int array) : bool = Array.for_all (fun c -> c <= 4) tiles
  ...
  then
    for mid = min_mid to Array.length melds - 1 do
      ...
      then (
        ...
        if distance < sht.(9) then dfs current target (m + 1) mid melds sht);
...
let write_table (filename : string) (length : int) : unit =
  ...
  Printf.printf
    "  Table size: %d entries x 10 bytes = %.2f MB\n"
    (List.length hands)
  ...
  let domains =
    Array.init num_domains (fun i ->
      ...
      let hands_chunk =
        if i = num_domains - 1
        then list_slice start (List.length hands - start) hands
        else list_slice start chunk_size hands
      in
      Domain.spawn (fun () ->
        let chunk_results =
          Array.map
            (fun (hand, _hash) -> calc_section_shanten hand melds)
  ...
  Array.iter
    (fun chunk_results ->
       Array.iter (fun sht -> Array.iter (fun v -> output_byte oc v) sht) chunk_results)
    results;
  close_out oc;
