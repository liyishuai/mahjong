...
let tile_of_string (s : string) : (tile, invalid_tile) result =
  ...
  with
  | Not_found -> Error (String s)
;;

let int_of_tile (t : tile) : int = t
let string_of_tile (t : tile) : string = mjai_pai_strings.(t)

(* Tile properties *)
let deaka (t : tile) : tile =
  if t = tile_id_5mr
  then tile_id_5m
  else if t = tile_id_5pr
  then tile_id_5p
  else if t = tile_id_5sr
  then tile_id_5s
  else t
;;
...
let akaize (t : tile) : tile =
  if t = tile_id_5m
  then tile_id_5mr
  else if t = tile_id_5p
  then tile_id_5pr
  else if t = tile_id_5s
  then tile_id_5sr
  else t
;;

let is_aka (t : tile) : bool = t = tile_id_5mr || t = tile_id_5pr || t = tile_id_5sr
let is_jihai (t : tile) : bool = t >= tile_id_E && t <= tile_id_C
...
let is_unknown (t : tile) : bool = t >= tile_id_unknown
...
let next (t : tile) : tile =
  ...
  then t
    ...
    if kind < 3
    then (kind * 9) + ((num + 1) mod 9)
    else if num < 4
    then (3 * 9) + ((num + 1) mod 4)
    else (3 * 9) + 4 + ((num - 4 + 1) mod 3))
;;
...
let prev (t : tile) : tile =
  ...
  then t
    ...
    let num = tile mod 9 in
    if kind < 3
    then (kind * 9) + ((num + 9 - 1) mod 9)
    else if num < 4
    then (3 * 9) + ((num + 4 - 1) mod 4)
    else (3 * 9) + 4 + ((num - 4 + 3 - 1) mod 3))
;;
