...
let hand_with_aka (s : string) : (int array, string) result =
  (* Check ASCII only *)
  if not (String.for_all (fun c -> Char.code c < 128) s)
  then Error "hand contains non-ASCII content"
  else (
    ...
    try
      String.iter
        (fun c ->
           match c with
           | '0' .. '9' -> stack := (Char.code c - Char.code '0') :: !stack
           | 'm' | 'p' | 's' | 'z' ->
             let kind =
               match c with
               | 'm' -> 0
             ...
             stack := []
           | ' ' | '\t' | '\n' -> ()
           | _ -> raise (Invalid_argument (Printf.sprintf "unexpected character '%c'" c)))
        s;
      (* Check for incomplete numbers at end *)
      if !stack <> [] then raise (Invalid_argument "incomplete number at end of string");
      Ok ret
    with
...
let tiles_to_string (tiles34 : int array) (aka : bool array) : string =
  if Array.length aka <> 3 then invalid_arg "aka array must have exactly 3 elements";
  let parts = ref [] in
  ...
  for num = 0 to 6 do
    ...
    if count > 0
    then jihai := !jihai ^ String.make count (char_of_int (Char.code '0' + num + 1))
  done;
  (* Join parts *)
