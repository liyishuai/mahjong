...
let calc (is_dealer : bool) (fu : int) (han : int) : point =
  let (ron, tsumo_ko, tsumo_oya) =
    if is_dealer then
      ...
      | _ ->
          invalid_arg
            (Printf.sprintf "impossible combination of %d fu and %d han" fu han)
    else
...
let yakuman (is_dealer : bool) (count : int) : point =
  if is_dealer then
    { ron = 48000 * count; tsumo_ko = 16000 * count; tsumo_oya = 0 }
  else
    { ron = 32000 * count; tsumo_ko = 8000 * count; tsumo_oya = 16000 * count }
