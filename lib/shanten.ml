...
let hash_suit (tiles : int array) (offset : int) : int =
  let rec loop i acc =
    if i >= offset + 9 then acc
    else loop (i + 1) (acc * 5 + tiles.(i))
  in
  loop offset 0
...
let shanten_tables : tables Lazy.t =
  lazy (
    ...
    let find_file basename =
      match List.find_map open_file [
        basename;
        "../lib/" ^ basename;
        "_build/default/lib/" ^ basename
      ] with
      | Some ic -> ic
    ...
    let load_table basename size =
      ...
      with exn ->
        close_in ic;
        raise (InitError (IoError (Printf.sprintf "Failed to read %s: %s"
          basename (Printexc.to_string exn))))
    in
...
let calc_normal_internal (tables : tables)
    (tiles34 : int array) (len_div3 : int) : int =
  ...
  let entry_man = suit_table.(hash_man) in
  let entry_pin = suit_table.(hash_pin) in
  let entry_sou = suit_table.(hash_sou) in
  let entry_honor = honor_table.(hash_honor) in
...
let calc_chitoi_public (tiles34 : int array) : t =
  calc_chitoi tiles34
...
let calc_kokushi_public (tiles34 : int array) : t =
  calc_kokushi tiles34
...
let calc_all (tiles34 : int array) (len_div3 : int) : t =
  ...
  if shanten_normal <= 0 || len_div3 < 4 then shanten_normal
  else
...
let to_string (shanten : t) : string =
  if shanten < 0 then "Winning"
  else if shanten = 0 then "Tenpai"
