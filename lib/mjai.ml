...
type event =
  ...
  | Dora of { dora_marker : int (* New dora indicator *) }
  | Reach of { actor : int }
  | Reach_accepted of { actor : int }
  | Hora of
      { actor : int (* Winner *)
  ...
  | Ryukyoku of { deltas : int array option (* Optional score deltas *) }
  | End_kyoku
  | End_game
...
let augment (ev : event) : event =
  ...
  | Tsumo { actor; pai } -> Tsumo { actor; pai = swap_tile pai }
  | Dahai { actor; pai; tsumogiri } -> Dahai { actor; pai = swap_tile pai; tsumogiri }
  ...
  | Daiminkan { actor; target; pai; consumed } ->
    Daiminkan
      { actor; target; pai = swap_tile pai; consumed = Array.map swap_tile consumed }
  | Kakan { actor; pai; consumed } ->
    Kakan { actor; pai = swap_tile pai; consumed = Array.map swap_tile consumed }
  | Ankan { actor; consumed } -> Ankan { actor; consumed = Array.map swap_tile consumed }
  | Dora { dora_marker } -> Dora { dora_marker = swap_tile dora_marker }
...
let no_meta (event : event) : event_ext = { event; meta = None }
...
module Json = struct
  ...
  let tile_to_json (t : int) : Yojson.Safe.t = `String (Tiles.string_of_tile t)
  ...
  let event_of_json (j : Yojson.Safe.t) : event =
    ...
    | "none" -> None
    | "start_game" ->
      let names = member "names" j |> to_list |> List.map to_string |> Array.of_list in
      let seed =
        try
    ...
    | "start_kyoku" ->
      Start_kyoku
        ...
        ; tehais =
            member "tehais" j
            |> to_list
            |> List.map (fun tehaiJson ->
              tehaiJson |> to_list |> List.map tile_of_json |> Array.of_list)
            |> Array.of_list
        }
    | "tsumo" ->
      Tsumo
    ...
    | "chi" ->
      Chi
        ...
        ; pai = member "pai" j |> tile_of_json
        ; consumed =
            member "consumed" j |> to_list |> List.map tile_of_json |> Array.of_list
        }
    | "pon" ->
      Pon
        { actor = member "actor" j |> to_int |> validate_actor
    ...
    | "dora" -> Dora { dora_marker = member "dora_marker" j |> tile_of_json }
    | "reach" -> Reach { actor = member "actor" j |> to_int |> validate_actor }
    | "reach_accepted" ->
      Reach_accepted { actor = member "actor" j |> to_int |> validate_actor }
    | "hora" ->
      Hora
        { actor = member "actor" j |> to_int |> validate_actor
        ; target = member "target" j |> to_int |> validate_actor
        ; deltas =
            (try
               Some (member "deltas" j |> to_list |> List.map to_int |> Array.of_list)
             with
             | _ -> Option.none)
        ; ura_markers =
            (try
               Some
                 (member "ura_markers" j
                  |> to_list
                  |> List.map tile_of_json
                  |> Array.of_list)
             with
             | _ -> Option.none)
        }
  ...
  let event_to_json (ev : event) : Yojson.Safe.t =
    ...
    | Start_game { names; seed } ->
      let fields =
        ...
        ; "names", `List (Array.to_list (Array.map (fun n -> `String n) names))
        ]
      in
      let fields =
        match seed with
      ...
      `Assoc
        ...
        ; ( "tehais"
          , `List
              (Array.to_list
                 (Array.map
                    (fun tehai -> `List (Array.to_list (Array.map tile_to_json tehai)))
                    tehais)) )
        ]
    | Tsumo { actor; pai } ->
    ...
    | Chi { actor; target; pai; consumed } ->
      `Assoc
        ...
        ; "target", `Int target
        ; "pai", tile_to_json pai
        ; "consumed", tiles_to_json consumed
        ]
    | Pon { actor; target; pai; consumed } ->
      `Assoc
        [ "type", `String "pon"
    ...
    | Reach { actor } -> `Assoc [ "type", `String "reach"; "actor", `Int actor ]
    | Reach_accepted { actor } ->
      `Assoc [ "type", `String "reach_accepted"; "actor", `Int actor ]
    | Hora { actor; target; deltas; ura_markers } ->
      let fields =
        [ "type", `String "hora"; "actor", `Int actor; "target", `Int target ]
      in
      let fields =
        match deltas with
        | Some d ->
          fields @ [ "deltas", `List (Array.to_list (Array.map (fun v -> `Int v) d)) ]
        | Option.None -> fields
      in
      let fields =
        match ura_markers with
  ...
  let event_from_string (s : string) : event = from_string s |> event_of_json

  (** Convert event to JSON string *)
  let event_to_string (ev : event) : string = event_to_json ev |> Yojson.Safe.to_string
end
