...
let test_parsing () =
  ...
  (match Hand.hand "1111m 333p 222s 444z" with
   | Ok tiles ->
       assert (tiles.(0) = 4);  (* four 1m *)
       assert (tiles.(11) = 3); (* three 3p *)
       assert (tiles.(19) = 3); (* three 2s *)
       assert (tiles.(30) = 3); (* three 4z (North) *)
   | Error _ -> assert false);

  (* Test with aka (red fives) *)
  (match Hand.hand_with_aka "22334450m234p2s3s4s" with
   | Ok tiles ->
       assert (tiles.(1) = 2);  (* two 2m *)
       assert (tiles.(2) = 2);  (* two 3m *)
       assert (tiles.(3) = 2);  (* two 4m *)
       assert (tiles.(4) = 1);  (* one 5m *)
       assert (tiles.(34) = 1); (* one 5mr *)
   | Error _ -> assert false);
