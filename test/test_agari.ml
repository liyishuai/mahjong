...
let test_is_agari_positive () =
  ...
  let winning_hands = [
    ...
    "111222333m 234s 11z";
    "112233m 112233p 11z";
    "11223344556677z";   (* Seven pairs *)
    "1133556699m 1122s"; (* Seven pairs *)
    "11m 345p";
    "11m 112233p";
    "11m 123456789p";
    "11m 111p 111s";
  ...
  List.iter (fun hand_str ->
    let tiles = parse_hand hand_str in
    if not (Agari.is_agari tiles) then begin
...
let test_is_agari_negative () =
  ...
  List.iter (fun hand_str ->
    let tiles = parse_hand hand_str in
    if Agari.is_agari tiles then begin
      Printf.printf "  FAILED: %s should not be agari\n" hand_str;
      assert false
    end
  ) non_winning_hands;

  Printf.printf "  Non-winning hands tests passed (%d hands)\n" (List.length non_winning_hands)
  ...
  for _ = 1 to max_tests do
    let rec generate_hand () =
      ...
      while !tiles_left > 0 do
        ...
        if hand.(idx) < 4 then (hand.(idx) <- hand.(idx) + 1; decr tiles_left)
      done;
    ...
    if result <> result_ref then (
      let aka = [| false; false; false |] in
      let hand_str = Hand.tiles_to_string hand aka in
      Printf.printf "  MISMATCH: is_agari=%b vs is_agari_ref=%b, Hand: %s\n"
        result result_ref hand_str;
